...
| Recording and receiving package | v0.3.0; record each installed commit from `narrowslink version --json` |
...
2. Draw the complete observation path and identify whether NarrowsLink receives the primary stream or a copy. Note unavailable upstream loss counters and OS-specific limits before starting.
